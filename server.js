...
async function renderChatList(chatId, messageId) {
  ...
  const keyboard = chats.map(c => ([{
    text: `${c.pinned ? '📌 ' : ''}${c.displayName}${c.lastMessage ? ' — ' + c.lastMessage.slice(0, 25) : ''}`,
...
bot.on('callback_query', async (query) => {
  ...
  try {
    if (data === 'cl:list') {
      ...
      const name = getChatDisplayName(id, snap.exists ? snap.data() : {});
      await bot.sendMessage(chatId,
        `✍️ এখন থেকে আপনার পরবর্তী মেসেজগুলো <b>${esc(name)}</b>-কে পাঠানো হবে।\nথামাতে বা অন্য কাউকে বেছে নিতে /stopchat বা /chats লিখুন।`,
        { parse_mode: 'HTML' }
      );
...
bot.onText(/^\/help(?:@\w+)?$/i, async (msg) => {
  ...
  await bot.sendMessage(msg.chat.id,
    ...
    `• /chats — সব লাইভ চ্যাট ভিজিটরের লিস্ট দেখুন, পিন/রিনেইম/ডিলিট করুন, বা মেসেজ পাঠানোর জন্য একজনকে বেছে নিন\n` +
...
bot.on('message', async (msg) => {
  ...
  // 2) Reply-to-notification flow (still supported)
  const replied = msg.reply_to_message;
  if (replied) {
    const target = await getReplyTarget(replied.message_id);
    if (!target) {
  ...
  // 3) A visitor was selected from /chats — send plain messages there
...
async function deleteOldDocsIn(collectionRef, cutoffTimestamp) {
  const snap = await collectionRef.where('timestamp', '<', cutoffTimestamp).limit(400).get();
