...
// Note: the TechFix website itself is deployed separately on Netlify — this
// process only runs the Telegram <-> Firestore bridge (notifications,
// live-chat replies, appointment status, auto-cancel, retention cleanup).
// /health exists purely so uptime checks have something to ping.
...
// Mirrors a status change into the public-safe "appointmentTracking/{referenceNumber}"
// record used by the website's "Track My Appointment" feature (added in the
// latest site update). Without this, statuses changed from Telegram (/status,
// or the 5-day auto-cancel) would never show up for the customer.
async function mirrorStatusToTracking(referenceNumber, status) {
  if (!referenceNumber) return; // older appointments booked before tracking existed
  try {
    await db.collection('appointmentTracking').doc(referenceNumber).set({
      status,
      statusUpdatedAt: FieldValue.serverTimestamp()
    }, { merge: true });
  } catch (e) {
    console.error('[mirrorStatusToTracking] failed for', referenceNumber, e.message);
  }
}
  ...
  try {
    ...
    for (const doc of apptSnap.docs) {
      ...
      if (st === 'Completed' || st === 'Cancelled') {
        await doc.ref.delete().catch(console.error);
        if (data.referenceNumber) {
          await db.collection('appointmentTracking').doc(data.referenceNumber).delete().catch(console.error);
        }
      }
    }
